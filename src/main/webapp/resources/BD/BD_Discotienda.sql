...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_NOMBRES` (IN `_id` INT)  NO SQL
SELECT nombre FROM cancionesdos WHERE id = _id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_PRECIO_DISCO` (IN `_id` INT)  NO SQL
SELECT precio FROM cancionesdos WHERE id = _id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_VISTA_CANCIONES` ()  SELECT cancionesdos.id, cancionesdos.nombre, cancionesdos.duracion, cancionesdos.precio, discosdos.album FROM cancionesdos, discosdos$$
